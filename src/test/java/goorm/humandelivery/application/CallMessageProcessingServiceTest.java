...
import goorm.humandelivery.application.customer.service.CallSaveService;
...
class CallMessageProcessingServiceTest {
    ...
    @BeforeEach
    void setUp() {
        customerLoadService = mock(CustomerLoadService.class);
        callSaveService = mock(CallSaveService.class);
        kafkaMessageQueueService = mock(KafkaMessageQueueService.class);

        service = new CallMessageProcessingService(customerLoadService, callSaveService, kafkaMessageQueueService);
    }
    ...
    void processMessage_성공_케이스() {
        ...
        when(customerLoadService.findCustomerByLoginId(senderId)).thenReturn(customer);
        ...
        // then
        verify(customerLoadService).findCustomerByLoginId(senderId);
    ...
    void processMessage_고객정보없음_예외() {
        ...
        when(customerLoadService.findCustomerByLoginId(senderId))
